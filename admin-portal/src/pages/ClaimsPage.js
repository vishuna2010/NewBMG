...
const ClaimsPage = () => {
  ...
  if (loading) return <MainLayout pageTitle="Claims" actions={actions}><p>Loading claims...</p></MainLayout>;
  // Show error prominently if claims list is empty due to it
  if (error && claims.length === 0) return <MainLayout pageTitle="Claims" actions={actions}><p style={{ color: 'red' }}>Error fetching claims: {error}</p></MainLayout>;
  ...
  return (
    <MainLayout pageTitle="Claims" actions={actions}>
